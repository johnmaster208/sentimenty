		...
		$('form#textpool-form-submit').submit(function(e) {
			...
			if($textpoolField.val().length == 0 || $textpoolField.val().length < 3){
				...
				$container.on('show.bs.modal',function(e){
					//if there is any leftover output from the last one, clear it.
				...
				$container.on('shown.bs.modal',function(e){
					$.ajax({
						...
						'success': function(data) {
							...
							$('div#accordion').remove();
							var resultsPanel = '<div class="panel-group col-xs-12 col-md-6 row" id="accordion" role="tablist" aria-multiselectable="true"><button class="col-xs-12 col-md-6 btn btn-default btn-primary">CHART</button></div>';
							$('#textpool-panel').append(resultsPanel);
							drawOutputDrawer(data,reqType);
							// var Data = data;
							// console.log(Data);
							// var chart = drawHighChart(Data,'pie');
							// if(chart !== 'undefined'){
							// 	$container.find('div.modal-body')
							// 	.html(chart);
							// }
